...
const parseDate = (dateString: string | null): Date | null => {
    ...
    if (parts.length === 3) {
        ...
        const date = new Date(Date.UTC(+parts[0], +parts[1] - 1, +parts[2])); // Використовуємо UTC для уникнення проблем з часовими зонами при парсингу
...
const ReportsPage: React.FC = () => {
    ...
    // const [categories, setCategories] = useState<CategoryInfo[]>([]); // Розкоментуй, якщо категорії знадобляться
        ...
        relevantTransactions.forEach(tx => {
            ...
            if (txDate) {
                ...
                if (!monthlyData[monthYear]) {
                     // Форматуємо назву місяця для осі X (напр., 'Кві 2025')
         ...
         // Конвертуємо в масив і сортуємо по ключу РРРР-ММ
    ...
    return (
        <div>
            ...
            {isLoading && <p className="mt-4">Завантаження звітів...</p>}
            ...
            {!isLoading && !error && (
                 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                    ...
                    <div className="p-4 border rounded shadow bg-white min-h-[350px]">
                    ...
                    <div className="p-4 border rounded shadow bg-white min-h-[350px]">
                         ...
                         {chartData.lineData.length > 0 ? (
                            <ResponsiveContainer width="100%" height={300}>
                               <LineChart data={chartData.lineData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                 ...
                                 <Legend />
                                 <Line type="monotone" dataKey="income" name="Надходження" stroke="#00C49F" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 6 }}/>
                                 <Line type="monotone" dataKey="expense" name="Витрати" stroke="#FF8042" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 6 }}/>
                               </LineChart>
                            </ResponsiveContainer>
