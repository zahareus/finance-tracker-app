...
interface MonthlyChartData {
    ...
    // Деталізація тепер не використовується напряму в Tooltip
...
const TransactionsPage: React.FC = () => {
    ...
    // Початкові дати - поточний місяць
    ...
    // Прибираємо SelectAll для категорій
    ...
    }, []); // Немає залежностей, бо використовує тільки new Date()
    ...
    // --- Компонент для Кастомної Підказки (Tooltip) - ВИМКНЕНО ТИМЧАСОВО ---
    ...
    return (
        <div>
          ...
          <div className="mb-6 p-4 border rounded bg-gray-50 space-y-4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3 items-end"> {/* Використовуємо Grid */}
                   {/* Колонка 1: Дати */}
                   {/* Об'єднуємо два інпути дати */}
                   <div className="flex flex-col sm:flex-row gap-2 md:col-span-1"> {/* Займає одну колонку */}
                   ...
                   <div className="md:col-span-1">
                       <label className="block text-xs font-medium text-gray-600 mb-1 invisible">Швидкі Періоди</label> {/* Невидимий лейбл для вирівнювання */}
                       <div className="flex space-x-2">
                           <button onClick={() => setDateRangePreset(30)} className="flex-1 px-2 py-1.5 text-xs border border-gray-300 rounded bg-white hover:bg-gray-50 shadow-sm">Місяць</button>
                           <button onClick={() => setDateRangePreset(90)} className="flex-1 px-2 py-1.5 text-xs border border-gray-300 rounded bg-white hover:bg-gray-50 shadow-sm">Квартал</button>
                           <button onClick={() => setDateRangePreset(365)} className="flex-1 px-2 py-1.5 text-xs border border-gray-300 rounded bg-white hover:bg-gray-50 shadow-sm">Рік</button>
                       </div>
                   </div>
                      ...
                      <div className="flex rounded border border-gray-300 overflow-hidden shadow-sm">
                        {(['Всі', 'Надходження', 'Витрата'] as const).map((type, index) => (
                            <button
                               ...
                               // Займають однакову ширину завдяки flex-1
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start pt-2">
                    ...
                    <div>
                       ...
                       {/* Прибрали фіксовану висоту і скрол */}
                        ...
                        <div className="border rounded p-2 bg-white space-y-1 shadow-sm">
                           {isLoading ? <p className="text-xs text-gray-400 p-1">Завантаження...</p> :
                            Array.isArray(categories) && categories.length > 0 ? categories
                                .filter(cat => cat.type === 'Надходження')
                                .map(cat => (
