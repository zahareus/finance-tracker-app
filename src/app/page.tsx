...
const parseDate = (dateString: string | null): Date | null => {
    ...
    console.warn("Could not parse date string:", dateString); // Попередження, якщо дата не розпізнана
    return null;
};
// --- Кінець хелперів ---
...
const TransactionsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
       setIsLoading(true);
       setError(null);
       console.log("Starting data fetch..."); // Діагностика
       try {
         const response = await fetch('/api/sheet-data');
         console.log("API response status:", response.status); // Діагностика
         if (!response.ok) {
           let errorText = `HTTP error! status: ${response.status}`;
           try {
               const errorData = await response.json();
               console.error("API error response:", errorData); // Діагностика
               errorText = errorData.error || errorText;
           } catch (e) {
               console.error("Failed to parse error JSON", e); // Діагностика
           }
           throw new Error(errorText);
         }
         const data = await response.json();
         console.log("Data received from API:", data); // Діагностика - ДУЖЕ ВАЖЛИВО!

         // Перевіряємо чи отримали масиви
         ...
         console.log("State updated with fetched data."); // Діагностика

       } catch (err) {
          console.error("Failed to fetch data in useEffect:", err); // Діагностика
          setError(err instanceof Error ? err.message : 'An unknown error occurred.');
       } finally {
         setIsLoading(false);
         console.log("Fetching finished, isLoading set to false."); // Діагностика
       }
    };
    fetchData();
  }, []);
  ...
  // --- Обробники змін фільтрів (залишаються такими самими) ---
  ...
  // --- РЕНДЕР КОМПОНЕНТА ---
  console.log("Rendering TransactionsPage. isLoading:", isLoading, "Error:", error); // Діагностика рендеру
  ...
  return (
    <div>
      ...
      {/* --- Таблиця транзакцій --- */}
      {/* ... (код таблиці залишається таким самим) ... */}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredTransactions.length === 0 ? (
                  ...
                  filteredTransactions.map((tx, index) => (
                    <tr key={`${tx.date}-${tx.account}-${tx.amount}-${index}`} className={`${tx.type === 'Витрата' ? 'bg-red-50 hover:bg-red-100' : 'bg-green-50 hover:bg-green-100'} transition-colors duration-150 ease-in-out`}> {/* Змінив key */}
