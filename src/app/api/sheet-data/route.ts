...
export async function GET() {
  try {
    ...
    const ranges = [
      ...
      'Categories!A2:B',
      'Agents!A2:A',
      'Projects!A2:D', // Колонки: A - назва, B - бонус з суми, C - бонус з балансу, D - статус
    ];
    ...
    if (!response.data.valueRanges || response.data.valueRanges.length === 0) {
        return NextResponse.json({ transactions: [], accounts: [], categories: [], counterparties: [], projects: [] });
    }
    ...
    const projects = rawProjects.map(row => ({
      ...
      status: row[3] ? String(row[3]).trim().toLowerCase() : '', // live, closed, або пусто
    })).filter(p => p.name);
